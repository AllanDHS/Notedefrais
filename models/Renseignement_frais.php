...
class Frais
{
    ...
    public static function getFrais(int $id_employes){
        ...
        try{
            ...
            $sql = "SELECT  FROM `renseignement_frais` INNER JOIN `type_de_frais` ON `renseignement_frais`.`id_type_de_frais` = `type_de_frais`.`name` INNER JOIN `status_note` ON `renseignement_frais`.`id_status_note` = `status_note`.`name` WHERE `renseignement_frais`.`id_employes` = :id_employes";
            // Préparation de la requête
            $stmt = $pdo->prepare($sql);
            // Bind des paramètres pour eviter les injections SQL
            $stmt->bindParam(':id_employes', $id_employes);
